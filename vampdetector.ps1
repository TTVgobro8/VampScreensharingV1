...
function Resolve-ModsPath {
    ...
    if ($instances.Count -gt 1) {
        Write-Host "    [!] Multiple running instances detected:" -ForegroundColor Yellow
        foreach ($inst in $instances) {
            Write-Host ("      * PID {0}: {1}" -f $inst.ProcessId, $inst.GameDir) -ForegroundColor Gray
        }
        Write-Host "    Using the first detected instance. Pass -ModsPath to target a specific one." -ForegroundColor DarkYellow
    }
